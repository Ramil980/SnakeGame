...
class longest_path(object):
    def __init__(self):
        ...
        self.recPredecessor = defaultdict(list)
        self.recDistance = defaultdict(list)
        self.recVisited = defaultdict(list)
        self.itemPosition = (0, 0)
        self.path = []
        self.totalPath = []

    def initializeRecVisited(self):
        for i in range(self.rows):
            for j in range(self.rows):
                self.recVisited[str((i, j))] = False

    def configure(self, snake):
        print(snake.head.pos, end=" ")
        print(self.itemPosition, end="\n")
        self.initializeRecVisited()
        if self.checkConnectedness(snake.head.pos, self.itemPosition):
            ...
            for i in range(len(self.path) - 1, 0, -1):
                if self.checkRecursiveConnectedness(self.path[i], self.path[i - 1]):
                    print("inside")
                    self.findRecursiveShortestDistance(self.path[i], self.path[i - 1])
                else:
                    self.totalPath.append(self.path[i-1])
            self.totalPath.insert(0, snake.head.pos)

            for i in range(len(self.totalPath)):
                print(self.totalPath[i], end=" ")
            print("\n\n")

            for i in range(len(self.totalPath) - 1):
                snake.directionX = self.totalPath[i + 1][0] - self.totalPath[i][0]
                snake.directionY = self.totalPath[i + 1][1] - self.totalPath[i][1]
                print("\n")
                #print(snake.directionX, end=" ")
                #print(snake.directionY, end=" ")
                #print(self.totalPath[i], end=" ")
                snake.turns[self.totalPath[i]] = [snake.directionX, snake.directionY]
    ...
    def checkRecursiveConnectedness(self, source, destination):
        queue = Queue()
        visited = defaultdict(list)
        for i in range(self.rows):
            for j in range(self.rows):
                visited[str((i, j))] = False
                self.recDistance[str((i, j))] = 1000
                self.recPredecessor[str((i, j))] = -1

        visited[str(source)] = True
        self.recDistance[str(source)] = 0
        queue.put(source)
        while not queue.empty():
            front = queue.get()
            for i in self.graph[str(front)]:
                if visited[str(i)] == False and self.recVisited[str(i)] == False:
                    if (str(i) != str(destination) and i in self.path):
                        pass
                    self.recDistance[str(i)] = self.recDistance[str(front)] + 1
                    self.recPredecessor[str(i)] = front
                    if str(i) == str(destination) and str(self.recPredecessor[str(i)]) == str(source):
                        pass
                    else:
                        visited[str(i)] = True
                        queue.put(i)
    ...
    def findRecursiveShortestDistance(self, source, destination):
        dest = destination
        tempPath = []
        tempPath.append(dest)
        ct = 0
        while self.recPredecessor[str(dest)] != -1:
            ct += 1
            tempPath.append(self.recPredecessor[str(dest)])
            dest = self.recPredecessor[str(dest)]

        for i in range(len(tempPath) - 2, -1, -1):
            self.totalPath.append(tempPath[i])
            print(tempPath[i], end = " ")
            self.recVisited[str(tempPath[i])] = True
        print("\n")
